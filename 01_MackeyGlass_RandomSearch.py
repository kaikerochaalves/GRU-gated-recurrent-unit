...
def build_model(filters = 64, kernel_size = 2, strides = 1, n_neurons=30, n_gru_hidden = 1, neurons_dense = 30, dropout_rate = 0, n_dense_hidden = 1, learning_rate=3e-3):
    ...
    model.add(keras.layers.Conv1D(filters=filters, kernel_size=kernel_size, strides=strides, padding="valid", input_shape=(X_train.shape[1],X_train.shape[2])))
    if n_gru_hidden == 1:
        model.add(keras.layers.GRU(n_neurons, return_sequences=False))
    elif n_gru_hidden == 2:
...
keras_reg = KerasRegressor(model=build_model, filters = 64, kernel_size = 2, strides = 1, n_neurons=30, n_gru_hidden = 1, neurons_dense = 30, dropout_rate = 0, n_dense_hidden = 1, learning_rate=3e-3)
...
# Results
GRU = f'{Model} & {NRMSE:.5f} & {NDEI:.5f} & {MAPE*100:.2f} & -'
print(f"\n{GRU}")
